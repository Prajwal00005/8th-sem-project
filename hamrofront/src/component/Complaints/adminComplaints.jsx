    ...
    complaints.length === 0 ? (
      ...
      <div className="divide-y divide-[#E8EFEA]">
        {complaints.map((complaint) => {
          ...
          const currentResponse = isResidentComplaint
            ? residentResponses[complaint.id] || ""
            : "";
          const currentStatus = isResidentComplaint
            ? residentStatuses[complaint.id] || "in_progress"
            : complaint.status;
                ...
                userRole === "admin" && (
                  <div className="mt-6 space-y-4">
                    <textarea
                      ...
                      onChange={(e) =>
                        setResidentResponse(complaint.id, e.target.value)
                      }
                    ...
                    <div className="flex items-center gap-4">
                      <select
                        value={currentStatus}
                        onChange={(e) =>
                          setResidentStatus(complaint.id, e.target.value)
                        }
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-medium text-[#2C3B2A]">
            Complaints Management
          </h2>
          <p className="text-md text-[#5C7361] mt-2">
            Handle resident complaints and submit issues to superadmin
          </p>
        </div>
        <button
        ...
        <button
          ...
          className={`px-6 py-2 rounded-lg font-semibold transition-all duration-200 shadow-sm ${
            activeTab === "resident"
              ...
              : "bg-[#E8EFEA] text-[#2C3B2A] hover:bg-[#D8E3DC]"
          }`}
        >
          Resident Complaints
        </button>
      ...
      <div className="bg-white rounded-xl shadow-md border border-[#E8EFEA] overflow-hidden">
        ...
        {activeTab === "resident"
          ? renderComplaintList(residentComplaints, "resident")
          : renderComplaintList(adminComplaints, "admin")}
      </div>
