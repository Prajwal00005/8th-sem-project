...
import { Pie, PieChart, Label, Line, LineChart, CartesianGrid, XAxis, YAxis, Tooltip, ResponsiveContainer } from 'recharts';
            ...
            try {
                ...
                const [statsRes, trendsRes, recentRes] = await Promise.all([
                    api.get('/api/v1/visitors/stats/', config).catch(err => ({
                        error: err.response?.data?.detail || 'Failed to fetch visitor stats'
                    })),
                    api.get('/api/v1/visitors/trends/', config).catch(err => ({
                        error: err.response?.data?.detail || 'Failed to fetch visitor trends'
                    })),
                    api.get('/api/v1/visitors/recent/', config).catch(err => ({
                        error: err.response?.data?.detail || 'Failed to fetch recent visitors'
                    }))
                ]);
                ...
                if (statsRes.error) {
                    ...
                    setVisitorStats(statsRes.data.map(item => ({
                        status: item.status ? item.status.charAt(0).toUpperCase() + item.status.slice(1) : 'Unknown',
                        count: item.count || 0,
                        fill: item.status === 'checked-in' ? '#90EE90' : item.status === 'checked-out' ? '#ADD8E6' : '#FECACA'
                    })));
                }
                ...
                return (
                    <div className="p-8 bg-[#F5F8F6]">
                        <div className="max-w-7xl mx-auto space-y-6">
                            <div>
                                <h2 className="text-2xl font-semibold text-[#2C3B2A]">Security Dashboard</h2>
                                <p className="text-sm text-[#5C7361] mt-1">Monitor and manage visitor activities</p>
                            </div>
                                ...
                                <>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                        <div className="bg-white rounded-xl shadow-sm border border-[#E8EFEA] p-6">
                                            <h3 className="text-lg font-medium text-[#2C3B2A] mb-4">Current Salary</h3>
                                            <p className="text-2xl font-bold text-[#395917]">
                                                {salaryInfo ? `₹${salaryInfo.current_salary}` : 'Loading...'}
                                            </p>
                                        </div>
                                        <div className="bg-white rounded-xl shadow-sm border border-[#E8EFEA] p-6 md:col-span-2">
                                            <h3 className="text-lg font-medium text-[#2C3B2A] mb-4">Recent Visitors</h3>
                                            {recentVisitors.length > 0 ? (
                                                <ul className="space-y-2">
                                                    {recentVisitors.map(visitor => (
                                                        <li key={visitor.id} className="flex justify-between text-sm text-[#5C7361]">
                                                            <span>{visitor.name}</span>
                                                            <span>{visitor.status} - {visitor.created_at}</span>
                                                        </li>
                                                    ))}
                                                </ul>
                                            ) : (
                                                <p className="text-sm text-[#5C7361]">No recent visitors</p>
                                            )}
                                        </div>
                                    </div>
