    ...
    <div className="space-y-4 p-4">
      ...
      <div className="bg-white/70 backdrop-blur-sm rounded-lg border border-white/50 p-4">
        <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
          <div>
            <h3 className="text-sm font-semibold text-slate-800">Filter Bills</h3>
            <p className="text-xs text-slate-500 mt-1">Filter by payment status</p>
          </div>
          <select
            value={statusFilter}
        ...
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white/95 backdrop-blur-sm rounded-xl shadow-xl w-full max-w-2xl mx-4 p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-xl font-semibold text-slate-800">Bill Details</h3>
                <p className="text-sm text-slate-500 mt-1">View detailed bill information</p>
              </div>
              <button
                type="button"
                onClick={closeModal}
            ...
            <div className="mb-6 space-y-3">
              <div className="flex justify-between py-2 border-b border-slate-100">
                ...
                <span className="text-sm font-medium text-slate-800">{selectedBill.room_number}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-slate-100">
                <span className="text-sm text-slate-600">Date</span>
                <span className="text-sm font-medium text-slate-800">{selectedBill.date}</span>
              </div>
              <div className="flex justify-between py-2 border-b border-slate-100">
                <span className="text-sm text-slate-600">Status</span>
                <span
                  className={`px-2 py-1 rounded-full text-xs font-medium ${
                    selectedBill.payment_status === "paid" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"
                  }`}
                >
            ...
            <div className="mb-6">
              <h4 className="text-sm font-semibold text-slate-800 mb-3">Bill Items</h4>
              <div className="bg-slate-50 rounded-lg overflow-hidden">
                <div className="overflow-x-auto">
                  <table className="w-full">
                    <thead>
                      <tr className="bg-slate-100 border-b border-slate-200">
                        <th className="px-4 py-2 text-left font-medium text-slate-700 text-xs">S.N.</th>
                        <th className="px-4 py-2 text-left font-medium text-slate-700 text-xs">Bill Name</th>
                        <th className="px-4 py-2 text-left font-medium text-slate-700 text-xs">Units</th>
                        <th className="px-4 py-2 text-left font-medium text-slate-700 text-xs">Rate / Unit</th>
                        <th className="px-4 py-2 text-left font-medium text-slate-700 text-xs">Amount</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-200">
                      {selectedBill.items.map((item, idx) => (
                        <tr key={item.id || idx} className="hover:bg-slate-50">
                          <td className="px-4 py-2 text-sm text-slate-800">{idx + 1}</td>
                          <td className="px-4 py-2 text-sm text-slate-800">{item.name}</td>
                          <td className="px-4 py-2 text-sm text-slate-600">{item.units ?? "-"}</td>
                          <td className="px-4 py-2 text-sm text-slate-600">{item.rate_per_unit ?? "-"}</td>
                          <td className="px-4 py-2 text-sm font-medium text-slate-800">₹{item.amount}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="mt-4 flex justify-between items-center pt-4 border-t border-slate-200">
                ...
                <span className="text-xl font-bold text-slate-800">₹{selectedBill.total_amount}</span>
              </div>
            </div>
