...
const AdminSubscriptionPayment = () => {
    ...
    const hasActiveSubscription = subscriptionPayments.some(
      (payment) => payment.subscription_year === currentYear && payment.status === 'active'
    );

    const handlePaySubscription = async () => {
        try {
            const response = await fetch('http://127.0.0.1:8000/api/v1/createAdminSubscriptionIntent/', {
                method: 'POST',
                headers: {
    ...
    const columns = [
        ...
        { label: 'End Date', key: 'subscription_end_date', render: (row) => (
          row.subscription_end_date ? new Date(row.subscription_end_date).toLocaleDateString() : 'N/A'
        )},
      ];
        ...
        <div className="p-8 bg-[#F5F8F6]">
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h2 className="text-2xl font-semibold text-[#2C3B2A]">Subscription Payment</h2>
                        <p className="text-[#5C7361] mt-1">Manage your annual subscription.</p>
                    </div>
                ...
                {/* Reminder Section */}
                {showReminder && subscriptionReminder && subscriptionReminder.reminder !== 'No subscription payments due within the next 7 days.' && (
                    <div className="bg-white border border-[#E8EFEA] rounded-xl p-6 shadow-sm flex items-center justify-between">
                        <div>
                            <p className="text-sm font-medium text-[#2C3B2A]">{subscriptionReminder.reminder}</p>
                ...
                {!showPaymentForm && (
                    <div className="bg-white rounded-xl shadow-sm border border-[#E8EFEA] overflow-hidden">
                        <div className="px-8 py-6">
                            <h3 className="text-xl font-semibold text-[#2C3B2A] mb-4">Subscription Details</h3>
                            <p className="text-base text-[#5C7361] mb-4">Yearly Subscription Price: ₹{subscriptionPrice}</p>
                            {hasActiveSubscription ? (
                                <div className="mb-4">
                                    <p className="text-base font-medium text-[#395917]">
                                        Your subscription for {currentYear} is active.
                                    </p>
                                    {subscriptionPayments.find(p => p.subscription_year === currentYear && p.status === 'active')?.subscription_end_date && (
                                        <p className="text-sm text-[#10bb35]">
                                            Valid until: {new Date(subscriptionPayments.find(p => p.subscription_year === currentYear && p.status === 'active').subscription_end_date).toLocaleDateString()}
                                        </p>
                                    )}
                                </div>
                            ) : (
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="bg-[#F5F8F6] text-[#2C3B2A]">
                                            ...
                                            <th className="px-8 py-4 text-left font-medium text-base">Subscription Year</th>
                                    ...
                                    <tbody className="divide-y divide-[#E8EFEA]">
                                        {subscriptionPayments.map((payment) => (
                                            <tr key={payment.id} className="hover:bg-[#F5F8F6] transition-colors">
                                                ...
                                                <td className="px-8 py-5 text-base text-[#2C3B2A]">
                                                    {payment.subscription_end_date ? new Date(payment.subscription_end_date).toLocaleDateString() : 'N/A'}
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
