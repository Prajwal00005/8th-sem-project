...
const RentPaymentForm = ({ paymentDetails, onClose }) => {
  ...
  const { loading, error, setLoading, setError, confirmPayment } = useRentPaymentStore();
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-xl max-w-md w-full mx-4">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-semibold text-[#2C3B2A]">Complete Payment</h3>
        <Button variant="secondary" onClick={onClose} icon={X} className="p-2 hover:bg-[#E8EFEA]" />
      </div>
      <div className="mb-6 space-y-2 text-[#5C7361]">
        <p>Resident: {paymentDetails.resident_name}</p>
        <p>Room: {paymentDetails.room_number}</p>
        <p>Period: {paymentDetails.period_from} to {paymentDetails.period_to}</p>
...
const RentPayment = () => {
  ...
  useEffect(() => {
    ...
    if (paymentReminder && paymentReminder.reminder !== 'No payment due at this time.') {
  ...
  const handlePeriodFromChange = (e) => {
    ...
    setPaymentDetails({...currentDetails, period_from: newFrom , period_to: ''});
    if (newFrom) {
  ...
  const monthsCovered =
    paymentDetails.period_from && paymentDetails.period_to
      ? Math.max(
          1,
          Math.round(
            (new Date(paymentDetails.period_to) - new Date(paymentDetails.period_from)) /
              (1000 * 60 * 60 * 24 * 30)
          )
        )
      : 1;
    ...
    <div className="p-8 bg-[#F5F8F6]">
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h2 className="text-2xl font-semibold text-[#2C3B2A]">Rent Payment</h2>
            <p className="text-[#5C7361] mt-1">Manage and track your rent payments</p>
          </div>
          <Button
        ...
        {showReminder && showPaymentReminder && paymentReminder && paymentReminder.reminder !== 'No payment due at this time.' && (
          <div className="bg-white border border-[#E8EFEA] rounded-xl p-6 shadow-sm flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-[#2C3B2A]">{paymentReminder.reminder}</p>
        ...
        {showRentIncreaseReminder && rentIncreaseReminder && (
          <div className="bg-white border border-[#E8EFEA] rounded-xl p-6 shadow-sm flex items-center justify-between">
            <div>
            <p className="text-sm font-medium text-[#2C3B2A]">{rentIncreaseReminder}</p>
              <p className="text-xs text-[#5C7361]">
                {rentIncreased && previousRent
                  ? `From ₹${previousRent.toFixed(2)} to ₹${monthlyRent.toFixed(2)}`
                  : `Current rent: ₹${monthlyRent.toFixed(2)}`}
                {lastRentIncreaseDate && ` (Last checked on ${new Date(lastRentIncreaseDate).toLocaleDateString()})`}
              </p>
            </div>
            <Button
              onClick={() => {
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-[#F5F8F6] text-[#2C3B2A]">
                  ...
                  <th className="px-8 py-4 text-left font-medium text-base">Period</th>
              ...
              <tbody className="divide-y divide-[#E8EFEA]">
                {paymentHistory.map((payment) => (
                  <tr key={payment.id} className="hover:bg-[#F5F8F6] transition-colors">
                    ...
                    <td className="px-8 py-5 text-base text-[#2C3B2A]">₹{payment.amount}</td>
        ...
        {showPaymentForm && clientSecret && (
          <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
            <Elements
              ...
              options={{ clientSecret, appearance: { theme: 'stripe', variables: { colorPrimary: '#395917' } } }}
            >
              <RentPaymentForm
                paymentDetails={{ ...paymentDetails, total_amount: totalAmount }}
        ...
        {showPaymentForm && !clientSecret && (
          <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50">
            <div className="bg-white p-8 rounded-xl shadow-xl max-w-md w-full mx-4">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-semibold text-[#2C3B2A]">Enter Payment Details</h3>
                <Button
                  variant="secondary"
              ...
              {monthlyRent === 0 ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="flex gap-4 justify-end">
                    ...
                    <Button
                      type="submit"
                      disabled={loading || !paymentDetails.period_from || !paymentDetails.period_to}
                      className="bg-[#395917] hover:bg-[#2C3B2A] text-white px-6 py-2.5"
                    >
