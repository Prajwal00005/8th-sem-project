...
export const useAdminDashboardStore = create((set, get) => ({
    ...
    setSubscriptionPayments: (payments) => set({ subscriptionPayments: payments }),
    setSubscriptionPrice: (price) => set({ subscriptionPrice: price }),
    setSubscriptionReminder: (reminder) => set({ subscriptionReminder: reminder }),
    ...
    fetchSubscriptionReminder: async () => {
        try {
            ...
            if (!response.ok) throw new Error('Failed to fetch subscription reminder');
            const data = await response.json();
