...
export const useSadminManagementStore = create((set, get) => ({
    admins: [],
    filteredAdmins: [],
    formData: { username: '', email: '', password: '', apartmentName: '',subscription_price:''},
    editingAdmin: null,
    isFormVisible: false,
    searchTerm: '',
    ...
    handleAddAdmin: async (event) => {
        ...
        try {
            const response = await axios.post('/api/v1/addAdmin/',{ ...formData,subscription_price: subscriptionPrice}, {
                headers: { Authorization: `Token ${localStorage.getItem('token')}` },
            });
            alert('Admin added successfully');
            set((state) => ({ admins: [...state.admins, response.data], filteredAdmins: [...state.admins, response.data] }));
            get().resetForm();
    ...
    handleEditAdmin: async (event) => {
        ...
        try {
            const response = await axios.put(`/api/v1/updateAdmin/${editingAdmin.id}/`, updatedAdmin, {
                headers: { Authorization: `Token ${localStorage.getItem('token')}` },
            });
            ...
            get().resetForm();
            set((state) => ({
                admins: state.admins.map((admin) => (admin.id === editingAdmin.id ? response.data : admin)),
                filteredAdmins: state.admins.map((admin) => (admin.id === editingAdmin.id ? response.data : admin))
            }));
            get().resetForm();
    ...
    handleDeleteAdmin: async (adminId, onConfirm) => {
        ...
        try {
            ...
            set((state) => ({
                ...
                filteredAdmins: state.filteredAdmins.filter((admin) => admin.id !== adminId)
            }));
    ...
    resetForm: () => set({ formData: { username: '', email: '', password: '', apartmentName: '',subscription_price:'' } }),

    startEditing: (admin) => {
        set({
            editingAdmin: admin,
            formData: { username: admin.username, email: admin.email, apartmentName: admin.apartmentName, password: '',subscription_price: admin.subscription_price? admin.subscription_price.toString() : ''  },
            isFormVisible: true
        });
    },
    ...
    toggleFormVisibility: () => {
        set((state) => {
            if (state.isFormVisible) {
                return { isFormVisible: false, editingAdmin: null, formData: { username: '', email: '', password: '', apartmentName: '',subscription_price: '' } };
            }
