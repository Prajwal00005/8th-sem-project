...
export const useSuperadminStripeSetupStore = create((set, get) => ({
  ...
  checkStripeStatus: async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/v1/stripe/connect/complete/",
        {
          method: "GET",
          headers: {
  ...
  handleStripeConnect: async () => {
    ...
    try {
      ...
      const response = await fetch(
        "http://127.0.0.1:8000/api/v1/stripe/connect/",
        {
          method: "GET",
          headers: {
            Authorization: `Token ${token}`,
  ...
  extendSubscriptionBy7Days: async (adminId) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/admins/${adminId}/extendSubscription7Days/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      set({ error: err.message || "Failed to extend subscription" });
    }
  },
  ...
  toggleAdminAccess: async (adminId, isActive) => {
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/admins/${adminId}/toggleAccess/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      set({ error: err.message || "Failed to update admin access" });
    }
  },
