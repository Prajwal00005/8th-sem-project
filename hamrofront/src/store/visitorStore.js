...
export const useVisitorStore = create((set, get) => ({
    isFormVisible: false,
    visitorData: { name: '', purpose: '', date: '', expected_time: '' },
    visitors: [],
    visitorHistory: [],
    loading: false,
    error: '',
    ...
    setVisitorData: (updates) => set((state) => ({ visitorData: { ...state.visitorData, ...updates } })),
    ...
    fetchVisitorHistory: async () => {
        ...
        try {
            ...
            const response = await axios.get(`/api/v1/visitors/history/?${params.toString()}`, {
                headers: { Authorization: `Token ${localStorage.getItem('token')}` },
            });
    ...
    handleAddVisitor: async (e) => {
        ...
        try {
            ...
            const response = await axios.post('/api/v1/visitors/register/', formattedData, {
                headers: { Authorization: `Token ${localStorage.getItem('token')}` },
            });
            ...
            set({ error: error.response?.data?.error || 'Failed to register visitor' });
        } finally {
            set({ loading: false });
        }
    },
    ...
    toggleFormVisibility: () => {
        set((state) => ({
            ...
            visitorData: !state.isFormVisible ? { name: '', purpose: '', date: '', expected_time: '' } : state.visitorData,
        }));
    },
